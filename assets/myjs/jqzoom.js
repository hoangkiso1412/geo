;(function($){
	$.fn.zoom = function(options){
		...
		var _option = {
			...
			thumb_image_height: 180,	// 当前展示图片的高
		...
		if($ul.is("ul") && $ul.children("li").length && $ul.find(".bzoom_big_image").length){
			...
			if(!$(".bzoom_small_thumbs").length){
				...
				for(var i=0; i<_option.small_thumbs; i++){
					...
					if(i==0){
						html += '<li class="bzoom_smallthumb_active"><img src="'+smurl+'" alt="small" style="width:'+smwidth+'px; max-width:80px; height:'+smheight+'px; max-height:60px;" /></li>';
					}else{
